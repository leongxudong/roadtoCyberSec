# Projects

This page tracks practical cybersecurity and GRC projects that can be developed into portfolio artefacts.

The goal is to show applied thinking: how security knowledge can be translated into usable templates, workflows, dashboards, learning tools, technical guides, and decision-support outputs.

## Technical Portfolio Projects

| Project | Repo | Status | Focus Area | Portfolio Value |
|---|---|---|---|---|
| MSMQ Security Lab | [MSMQ-Vulnerability](https://github.com/leongxudong/MSMQ-Vulnerability) | In Progress | Vulnerability validation / detection | Lab evidence, packet capture, Windows logs, Snort and Suricata notes |
| Splunk Windows Security Monitoring | [splunk-windows-security-monitoring](https://github.com/leongxudong/splunk-windows-security-monitoring) | Placeholder Created | SIEM / Windows monitoring | Windows Event ID mapping, SPL searches, authentication and admin activity monitoring |
| Entra Conditional Access Baseline | [entra-conditional-access-baseline](https://github.com/leongxudong/entra-conditional-access-baseline) | Placeholder Created | Identity security / Zero Trust | Conditional Access baseline policies, break-glass account notes, rollout checklist |
| Microsoft Purview Data Protection Guide | [microsoft-purview-data-protection-guide](https://github.com/leongxudong/microsoft-purview-data-protection-guide) | Placeholder Created | Data protection / compliance | Sensitivity labels, DLP policy design, test mode, rollout planning |
| Microsoft AZ-500 Study Guide and Labs | [microsoft-az-500-study-guide](https://github.com/leongxudong/microsoft-az-500-study-guide) | Published | Azure security engineering | Objective map, four-week plan, identity/network/workload/Defender/Sentinel labs, scenarios, and SC-500 transition |
| Microsoft SC-100 Cybersecurity Architect Study Guide | [microsoft-sc-100-study-guide](https://github.com/leongxudong/microsoft-sc-100-study-guide) | Published | Microsoft security architecture | Objective map, six-week plan, Zero Trust/resilience/operations/infrastructure/application/data/AI design labs and decision templates |
| Wireshark Network Analysis Basics | [wireshark-network-analysis-basics](https://github.com/leongxudong/wireshark-network-analysis-basics) | Placeholder Created | Network analysis | TCP, DNS, HTTP, TLS, and useful Wireshark display filters |
| Sysmon Sigma Detection Lab | [sysmon-sigma-detection-lab](https://github.com/leongxudong/sysmon-sigma-detection-lab) | Placeholder Created | Endpoint detection | Sysmon event mapping and Sigma-style detection documentation |
| Zero-Cost ISMS Template | [zero-cost-ISMS-template](https://github.com/leongxudong/zero-cost-ISMS-template) | Placeholder Created | ISMS / GRC / resilience | ISMS component map, BIA, BCP, DRP, risk register, internal audit checklist |

---

## GRC and AI-Assisted Project Ideas

| Project | Status | Focus Area | Portfolio Value |
|---|---|---|---|
| SETA Scenario Generator | Planned | Security awareness and human risk | Shows ability to translate policies into scenario-based learning |
| Vendor Risk Dashboard | Planned | Third-party risk management | Shows risk classification, tracking, and reporting capability |
| Agreement Assessment Bot | Planned | Contract and vendor assurance | Shows practical AI-assisted gap analysis concept |
| Cyber Maturity Heatmap | Planned | Cyber maturity and control assessment | Shows roadmap and management reporting capability |

---

## SETA Scenario Generator

### Concept

A tool that takes a security policy or control requirement and generates scenario-based security awareness questions, answers, and rationales.

### Problem It Addresses

Many security awareness programmes are generic. Staff may complete training without understanding how security policies apply to their day-to-day work.

### Possible Features

- Upload or paste policy text
- Generate role-based scenarios
- Generate multiple-choice questions
- Provide answer rationales
- Map questions to policy clauses or control themes
- Produce microlearning content for newsletters or staff briefings

### Skills Demonstrated

- Security awareness design
- Policy interpretation
- Human risk reduction
- AI-assisted content generation
- Practical GRC communication

---

## Vendor Risk Dashboard

### Concept

A lightweight dashboard for tracking vendor security assessments, risk ratings, due dates, review status, and outstanding remediation actions.

### Problem It Addresses

Vendor security reviews can become inconsistent if assessment depth, evidence, risk rating, and follow-up are not tracked in a structured way.

### Possible Features

- Vendor inventory
- Risk classification
- Data sensitivity indicator
- Service criticality rating
- Assessment status
- Contract/SLA review status
- Corrective action tracking
- Review due dates

### Skills Demonstrated

- Third-party risk management
- Risk scoring
- Dashboard design
- Control assurance tracking
- Management reporting

---

## Agreement Assessment Bot

### Concept

An AI-assisted tool that compares a vendor agreement, SLA, or service terms against an internal information security clause checklist.

### Problem It Addresses

Vendor agreements may omit important security, data protection, incident notification, availability, audit, or exit clauses. Manual review can be inconsistent and time-consuming.

### Possible Features

- Upload or paste contract/SLA text
- Compare against internal security clause checklist
- Flag missing or weak clauses
- Suggest mitigation controls if contract change is not possible
- Produce a risk summary for review
- Map gaps to risk owners such as Legal, IT, Operations, or Business

### Skills Demonstrated

- Contract risk review
- Vendor assurance
- AI-assisted analysis
- Security clause mapping
- Risk-based decision support

---

## Cyber Maturity Heatmap

### Concept

A dashboard or template that maps current cybersecurity controls against a maturity framework such as CIS Controls, NIST CSF, or an internal maturity model.

### Problem It Addresses

Management often needs a simple view of cybersecurity maturity, gaps, planned improvements, and residual risk.

### Possible Features

- Control domain heatmap
- Current control/evidence field
- Planned roadmap field
- Gap to validate field
- Maturity status labels
- Management summary view

### Skills Demonstrated

- Cyber maturity assessment
- Security roadmap planning
- Control gap analysis
- Executive reporting

---

## Notes

All project examples should be sanitized and generic. They should not include confidential employer data, client names, real vendor findings, sensitive system details, or proprietary materials.
